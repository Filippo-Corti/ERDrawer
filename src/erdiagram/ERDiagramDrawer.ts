...
export class ERDiagramDrawer {
    ...
    constructor(drawer: Drawer, erDiagram: ERDiagram) {
        this.drawer = drawer;
        this.erDiagram = erDiagram;
    }

    //Draws Diagram on the Drawer canvas
    drawDiagram() {
        this.erDiagram.nodes.forEach(v => {
            v.size = this.calcNodeSize();
        });
        this.drawer.clear();
        this.erDiagram.draw(this.drawer.ctx);
        //this.drawer.drawGrid(ERDiagramDrawer.DELTA);
    }
    ...
    */
    positionElegantly(numberOfGraphs : number, iterationsPerGraph : number): void {
        ...
        let minGraph = this.erDiagram.clone();

        for (let i = 0; i < numberOfGraphs; i++) {
            // Generate New Random Graph 
            this.erDiagram.randomizePositions(BORDER,  this.drawer.width - BORDER, BORDER, this.drawer.height - BORDER);
            ...
            if (crossings < minCrossings && !this.doesAnyEdgeCrossANode()) {
                ...
                minGraph = this.erDiagram.clone();
            }
        }

        this.erDiagram = minGraph.clone();
    ...
    executeFructhermanReingold(n: number, borderSize: number, centerMainNode: boolean = false): void {
        ...
        const k = Math.sqrt(area / this.erDiagram.nodes.size)
        let t = 5.0;

        const f_a = (x: number) => x * x / k;
        const f_r = (x: number) => k * k / x;

        const centralNode = this.erDiagram.getNodeWithMostEdges();
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (const [_, v] of this.erDiagram.nodes) {
                v.disp = new Vector2D(0, 0);
                for (const [_, u] of this.erDiagram.nodes) {
                    if (v == u) continue;
            ...
            for (const e of this.erDiagram.edges) {
    ...
    discretizeNodesCoordinates(borderSize: number): void {
        const rows = Math.floor(this.drawer.height / ERDiagramDrawer.DELTA + 1);
        const cols = Math.floor(this.drawer.width / ERDiagramDrawer.DELTA + 1);
        var nodesPerSquare: Node[][] = Array.from({ length: rows * cols }, () => []);
        for (const [_, v] of this.erDiagram.nodes) {
            const discrCoords = this.getDiscretizedCoords(v.pos, borderSize);
            const nodeCol = Math.round(discrCoords.x / ERDiagramDrawer.DELTA);
            const nodeRow = Math.round(discrCoords.y / ERDiagramDrawer.DELTA);
    ...
    private getDiscretizedCoords(v: Vector2D, borderSize: number): Vector2D {
        return this.getCoordsWithBoundaries(new Vector2D(Math.round(v.x / ERDiagramDrawer.DELTA) * ERDiagramDrawer.DELTA, Math.round(v.y / ERDiagramDrawer.DELTA) * ERDiagramDrawer.DELTA), borderSize);
    }
    ...
    private calcNodeSize(): number {
        ...
        const nodesCount = this.erDiagram.nodes.size;
    ...
    countCrossings(): number {
        let count: number = 0;
        for (let i = 0; i < this.erDiagram.edges.length; i++) {
            const e = this.erDiagram.edges[i];
            const s1 = e.getDrawingSegment();

            for (let j = i + 1; j < this.erDiagram.edges.length; j++) {
                const f = this.erDiagram.edges[j];
